...
contract NetswapERC20 {
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'Netswap: INVALID_SIGNATURE');
...
contract NetswapPair is NetswapERC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'Netswap: K');
        }
    ...
    function sync() external lock {
        _update(IERC20Netswap(token0).balanceOf(address(this)), IERC20Netswap(token1).balanceOf(address(this)), reserve0, reserve1);
    }
}
...
contract NetswapFactory is INetswapFactory {
    ...
    constructor(address _feeToSetter) public {
        ...
        initCodeHash = keccak256(abi.encodePacked(type(NetswapPair).creationCode));
    }
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        ...
        require(getPair[token0][token1] == address(0), 'Netswap: PAIR_EXISTS'); // single check is sufficient
    ...
    function getReserves(address tokenA, address tokenB) public override view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = NetswapPair(pairFor(tokenA, tokenB)).getReserves();
